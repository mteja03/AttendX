            ...
            <div className="flex flex-wrap items-center gap-2 px-5 py-2.5 border-b border-gray-100 bg-gray-50/50">
              ...
              {safeAudit.checkInSelfie && (
                <button type="button" onClick={async () => {
                  if (!safeAudit.checkInSelfie?.storagePath) return;
                  try {
                    const fileRef = storageRef(storage, safeAudit.checkInSelfie.storagePath);
                    const blob = await getBlob(fileRef);
                    const blobUrl = URL.createObjectURL(blob);
          ...
          <div className="bg-white rounded-2xl overflow-hidden max-w-sm w-full shadow-xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
              <div>
                ...
                <p className="text-xs text-gray-400">{safeAudit.auditorName} · {safeAudit.branch || safeAudit.location || '—'}</p>
              </div>
            ...
            <div className="px-4 py-3 border-t border-gray-100 space-y-1.5">
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-[10px] text-gray-500">
                {safeAudit.locationCheck?.lat != null && safeAudit.locationCheck?.lng != null && (
                  <div className="col-span-2"><span className="text-gray-400">Auditor GPS:</span> {safeAudit.locationCheck.lat.toFixed(6)}, {safeAudit.locationCheck.lng.toFixed(6)}</div>
                )}
                {safeAudit.locationCheck?.branchName && (
                  <div><span className="text-gray-400">Branch:</span> {safeAudit.locationCheck.branchName}</div>
                )}
              ...
              {(() => {
                const ts = safeAudit.locationCheck?.timestamp || safeAudit.checkInSelfie?.timestamp;
                if (!ts) return null;
